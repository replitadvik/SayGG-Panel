...
class Auth extends BaseController
{
    ...
    private function login_action()
    {
        ...
        if ($blockedRemain > 0) {
            ...
            return redirect()->route('/')->withInput()->with('msgDanger',
    ...
    public function register_action()
{
    ...
    $form_rules = [
        ...
        'telegram_chat_id' => [
            ...
            'rules' => 'required|numeric|min_length[6]|max_length[20]|is_unique[users.telegram_chat_id]',
            'errors' => [
                'is_unique' => 'The {field} has been taken.',
    ...
    public function forgot_password()
    {
        ...
        if ($this->request->getPost())
            return $this->forgot_password_action();
    ...
    /**
     * NEW: Forgot Password Action - Send OTP
     */
    private function forgot_password_action()
    {
        ...
        if (!$this->validate($form_rules)) {
            return redirect()->route('forgot-password')->withInput()->with('msgDanger', 'Please enter a valid username.');
        }

        $cekUser = $this->userModel->getUser($username, 'username');

        if ($cekUser) {
            // Check if user has Telegram chat ID for OTP
            if (empty($cekUser->telegram_chat_id)) {
                return redirect()->route('forgot-password')->withInput()->with('msgDanger', 'Telegram Chat ID not found. Please contact administrator for password reset.');
            }
            ...
            // Save pending password reset info in session
            ...
            if (!$sent) {
                ...
                return redirect()->route('forgot-password')->withInput()->with('msgDanger', 'Could not send OTP to Telegram. Please try again.');
            }

            return redirect()->to('reset-password')->with('msgInfo', 'Enter the OTP sent to your Telegram to reset your password.');
        }

        return redirect()->route('forgot-password')->withInput()->with('msgDanger', 'User not found.');
    }

    /**
     * NEW: Reset Password - Step 2: Verify OTP and Set New Password
     */
    public function reset_password()
    {
        $pending = session('pending_password_reset');

        // No pending password reset => back to forgot password
        if (!$pending || empty($pending['user_id'])) {
            return redirect()->route('forgot-password')->with('msgDanger', 'Session expired. Please request OTP again.');
        }
        ...
        if ($this->request->getMethod() === 'post') {
            ...
            if (!$this->validate($form_rules)) {
                return redirect()->to('reset-password')->withInput()->with('msgDanger', 'Please check the form.');
            }

            // Expired?
            if (time() > (int)$pending['expires']) {
                session()->remove('pending_password_reset');
                return redirect()->route('forgot-password')->with('msgDanger', 'OTP expired. Please request again.');
            }

            // Wrong OTP?
            if (!password_verify($otpInput, (string)$pending['otp_hash'])) {
                return redirect()->to('reset-password')->withInput()->with('msgDanger', 'Invalid OTP. Please try again.');
            }
            ...
            if (!$cekUser) {
                session()->remove('pending_password_reset');
                return redirect()->route('forgot-password')->with('msgDanger', 'User not found.');
            }

            // Hash and update new password
            $hashPassword = create_password($newPassword);
            $this->userModel->update($cekUser->id_users, ['password' => $hashPassword]);
            ...
            $tg->sendMessage(
                ...
                "If you didn't perform this action, please contact administrator immediately."
            );

            return redirect()->to('login')->with('msgSuccess', 'Password reset successfully. You can now login with your new password.');
        }
    ...
    public function logout()
    {
        if (session()->has('userid')) {
            ...
            $unset = ['userid', 'unames', 'time_login', 'time_since', 'user_level'];
