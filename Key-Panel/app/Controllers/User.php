...
class User extends BaseController
{
    ...
    public function ExtendDuration()
    {
        ...
        $duration = strtoupper(trim((string)$this->request->getPost('duration')));
        ...
        // Validate format like 30D or 12H
        if (!preg_match('/^\d+[DH]$/', $duration)) {
            return $this->response->setJSON([
                'success'   => false,
        ...
        $dbKey = $model->where('user_key', $userKey)->first();
        ...
        // Permission check
        if ((int)$user->level !== 1 && $dbKey['registrator'] !== $user->username) {
            return $this->response->setJSON([
                'success'   => false,
        ...
        $addHours = ($type === 'D') ? $value * 24 : $value;
        ...
        $currentExpiry = $dbKey['expired_date'];
        $newDuration   = ((int)$dbKey['duration']) + $addHours;
    
        // 🔥 CASE 1: Key NOT activated yet (no expiry)
        if (empty($currentExpiry)) {
    
            $updated = $model->update($dbKey['id_keys'], [
            ...
            return $this->response->setJSON([
                'success'        => true,
                'new_expiry'     => null, // keep null
                'total_duration' => $newDuration,
                'csrf_hash'      => csrf_hash()
            ]);
        }
    
        // 🔥 CASE 2: Key already activated
        $baseTime = (strtotime($currentExpiry) > time())
            ? strtotime($currentExpiry)
            : time();
    
        $newExpiryTimestamp = strtotime("+{$addHours} hours", $baseTime);
        $newExpiryDate      = date('Y-m-d H:i:s', $newExpiryTimestamp);
    
        $updated = $model->update($dbKey['id_keys'], [
            'expired_date' => $newExpiryDate,
            'duration'     => $newDuration
        ]);
