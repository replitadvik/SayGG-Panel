    ...
    private function user_edit_action()
{
    ...
    // Add email and telegram_chat_id validation only for level 1 users
    if ((int)$acting->level === 1) {
        $form_rules['telegram_chat_id'] = [
            'label'  => 'telegram chat id',
            'rules'  => 'permit_empty|numeric|max_length[20]',
            'errors' => ['numeric' => 'The {field} must contain only numbers.']
        ];
    }
    ...
    if (!$this->validate($form_rules)) {
        ...
        // Add email and telegram_chat_id only for level 1 users
        if ((int)$acting->level === 1) {
            $data_update['email'] = $this->request->getPost('email');
            $data_update['telegram_chat_id'] = $this->request->getPost('telegram_chat_id');
        }
    ...
    private function passwd_act()
{
    ...
    if (verify_password($password, $user->password)) {
        $validation->setError('password', 'New password cannot be same as current password.');
    }

    // Check password strength
    $strength = get_password_strength($password);
    if ($strength < 7) {
        $validation->setError('password', 'Password is too weak. Use uppercase, lowercase, numbers, and special characters.');
    }
    ...
    $form_rules = [
        ...
        'password2' => [
            ...
            'errors' => ['matches' => 'Password confirmation does not match.']
        ]
    ];

    // Custom strong password validation
    $validation->setRule('password', 'New Password', 
        'required|min_length[12]|max_length[100]|regex_match[/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]/]',
        [
            'regex_match' => 'Password must include: uppercase letter, lowercase letter, number, and special character (@$!%*?&)'
        ]
    );

    if (!$validation->withRequest($this->request)->run() || $validation->getErrors()) {
        return redirect()->back()->withInput()->with('msgDanger', 'Please fix the errors below.');
    }

    // Update to new secure password
    $newPasswordHash = create_password($password);
    $this->model->update(session('userid'), ['password' => $newPasswordHash]);
    
    return redirect()->back()->with('msgSuccess', 'Password successfully changed with enhanced security.');
}
