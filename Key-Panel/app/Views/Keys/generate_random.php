...
<style>
  ...
  /* ===== Modal ===== */
  .glass-modal-backdrop{ position:fixed; inset:0; background:rgba(0,0,0,.55); display:none; align-items:center; justify-content:center; z-index:1050; }
  .glass-modal{ width:min(94vw, 560px); border-radius:14px; overflow:hidden; background: var(--glass-bg); border:1px solid var(--glass-border); box-shadow: 0 18px 48px rgba(0,0,0,.55); backdrop-filter: blur(16px); transform: translateY(12px); opacity:0; transition:.25s; }
  .glass-modal.show{ transform: translateY(0); opacity:1; }
  .modal-top{ background: rgba(20,20,26,.85); padding:14px 16px; display:flex; align-items:center; justify-content:space-between; }
  ...
  .keys-box{ background:#070a0a; color:#3CFF93; border:1px solid rgba(255,255,255,.15); border-radius:10px; padding:12px 14px; font-family: ui-monospace, SFMono-Regular, Menlo, monospace; min-height:50px; max-height:100px; overflow:auto; white-space: pre-wrap; font-size:14px; }

  .glass-modal-actions{ padding:12px 16px; display:flex; gap:10px; align-items:center; justify-content:space-between; background: rgba(255,255,255,.04); }
  .btn-outline{ background:transparent; color:#fff; border:1px solid var(--glass-border); border-radius:10px; padding:10px 14px; font-weight:600; }
  .btn-outline:hover{ border-color:rgba(255,255,255,.45); }
  .btn-dark{ background: rgba(20,20,26,.9); color:#fff; border:none; border-radius:12px; padding:12px 16px; font-weight:700; }
...
<div class="glass-page">
  ...
  <div class="glass-grid">
    <!-- LEFT: CREATE LICENSE (Random only) -->
    <div class="glass-panel">
      <div class="glass-head">
        <div class="title"><i class="bi bi-key-fill me-2"></i>Create License (Random)</div>
    ...
    <div class="glass-panel">
      ...
      <div class="glass-body" style="color:var(--muted);font-size:14px;">
        Price = <em>devices × unit price (duration)</em>.<br>
...
<script>
  ...
  // Modal open from flashdata (same as original)
  <?php
  ...
  document.addEventListener('DOMContentLoaded', function(){
    if(flashKey){
      ...
      const computed = (!isNaN(unit) ? Number((Number(flashDev||1) * unit).toFixed(2)) : null);
      const finalCost = (flashTotal && !isNaN(Number(flashTotal))) ? Number(Number(flashTotal).toFixed(2)) : computed;
      openModal({ game: flashGame, dur: flashDur, dev: flashDev, cost: finalCost, key: String(flashKey).trim() });
    }
  });

  function openModal(meta){
    document.getElementById('mGame').textContent     = meta.game ?? '-';
    document.getElementById('mDuration').textContent = meta.dur ? (meta.dur + (isNaN(meta.dur) ? '' : ' Hours')) : '-';
    document.getElementById('mDevices').textContent  = meta.dev ?? '-';
    document.getElementById('mCost').textContent     = (meta.cost!=null && !isNaN(meta.cost)) ? ('$ ' + Number(meta.cost).toFixed(2)) : '-';
  ...
  function closeModal(){
    ...
    setTimeout(()=> backdrop.classList.remove('show-flex'), 200);
  }
  ...
  function downloadKeys(){
    ...
    a.href = url; a.download = `key-${ts}.txt`;
